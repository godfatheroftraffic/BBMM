...
3. Set domain name
4. Add footer links (privacy, terms, contact URLs)
5. Set AdKernel campaign ID (get from AdKernel dashboard)
...
```
/
├── index.html              → Master echo landing page
├── thank-you.html          → Master thank you page
├── /public/                → Image assets folder
│   └── (place images here)
└── README.md               → This file
```
...
1. **Reads URL parameters** (up to 182)
2. **Stores in cookies** (7-day expiration)
...
2. **Builds complete data layer** (all 182 params)
...
1. **Event - echo_page_view**
   - Type: Custom Event
   - Event name: `echo_page_view`

2. **Event - thankyou_conversion**
   - Type: Custom Event
   - Event name: `thankyou_conversion`

### **Tags (2 minimum):**

1. **AdKernel XML - Conversion Tracking**
   - Fires on: `thankyou_conversion`
   - URL: `https://xml.amplifai.it.com/conversion?id={{DLV - goal_id}}&c={{DLV - conversion_id}}&count=1&value={{DLV - conversion_value}}`

2. **AdKernel DSP - Conversion Tracking**
   - Fires on: `thankyou_conversion`
   - URL: `https://rtb2-useast.amplifai.it.com/conversion?cid={{DLV - conversion_id}}&step={{DLV - dsp_step}}`
...
- `archetype` - AT1, AT2, AT3, AT4, AT5
- `angle` - AG1, AG2, AG3, AG4, AG5
- `phase` - P1, P2, P3, P4, P5, P6, P7
- `variant` - Headline/ad variant number
...
- Verify `conversion_id` and `goal_id` are present
...
It captures **182 parameters** across:
